...
void App::ValidTask() {
    ...
    switch (m_Phase) {
        case Phase::CHANGE_CHARACTER_IMAGE:
            if (m_IceCream->GetImagePath() == GA_RESOURCE_DIR"/Image/Character/giraffe.png") {
                m_Phase = Phase::ABLE_TO_MOVE;
                m_IceCream->SetPosition({0, 0});
                m_IceCream->SetVisible(true);

                m_PRM->NextPhase();
            } else {
                LOG_DEBUG("The image is not correct");
                LOG_DEBUG("The image path is {} instead.", m_IceCream->GetImagePath());
            }
            break;
